...
(минимальные права, см. `sql/grants.sql`).

**Автоматический тест:** `tests/test_scenarios.py` — все 24 шага по порядку,
приложение подключается к тестовой базе под `bugtracker_app` (права выдаёт
тот же `sql/grants.sql`). Запуск: `pytest tests/test_scenarios.py -v`.
Нужен `APP_DATABASE_URL` в `.env`, иначе тест пропускается.
